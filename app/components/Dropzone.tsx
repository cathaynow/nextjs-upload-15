...
export function Dropzone() {
  ...
  return (
    <>
      <div
        {...getRootProps({
          className: "p-16 mt-10 border-dashed border-2 rounded-lg w-full",
        })}
      >
        <input {...getInputProps()} />
        {isDragActive ? (
      ...
      <div className="mt-6  grid  gird-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4">
        {files.map(({ file, uploading, id }) => (
          <div key={file.name} className="relative w-full group">
            ...
            <p className="mt-2 text-sm text-gray-500 truncate text-center justify-center">
              {file.name}
            </p>
          </div>
        ))}
